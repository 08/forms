 ...
 */
Form.prototype.onBuild(callback) {
    if (this.def) {
        callback();
    }
    else {
 ...
 */
Form.prototype.triggerOnBuild() {
    for (callback in this.onBuildCallbacks) {
        callback();
    }
}
    ...
    case 'GET':
        ...
        if (parse(request.url, 1).query) {
            ...
            this.onBuild(function() {
                that.submit();
                that.response.redirect(that.def.redirect);
            });
        }
        // If no data has been submitted, render form.
        else {
            this.onBuild(function() {
                that.def.render(that, that.request, that.response, that.next);
            });
        }
        return;
    case 'POST':
