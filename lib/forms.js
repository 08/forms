...
Form.load = function() {
    ...
    loaders.push(function(req, res, next) {
        if (that.prototype.CSRF && (req.method == 'POST') && req.session.user) {
          req.session.forms = req.session.forms || {}
          var build_id = req.body._build_id;

          if (req.session.forms[build_id]) {
            req.form = req.session.forms[build_id];
...
Form.on('build', function(req, res) {
  if (this.CSRF && req.session) {
      ...
      this.timeout(req.session, this.CSRF_timeout || 3600);

      req.session.forms[this.build_id] = this;
  }
});
...
Form.prototype.timeout = function (session, seconds) {
  var that = this;

  var now = Math.round(new Date().getTime() / 1000);

  if (this.build_id) {
    if ((now - this.last_time) >= seconds) {
      delete session.forms[this.build_id];
    }
    else {
      // Check the session again in 5 minutes.
      setTimeout(function() { that.timeout(session, seconds); }, 30000);
    }
  }
}
