 ...
 */
Form.prototype.handle = function(request) {
    var that = this;
    switch (request.method) {
    case 'GET':
        this.bind(parse(obj.url, 1).query);
        this.data = true;
        this.submit();
        return;
    case 'POST':
        ...
        request.addListener('end', function(){
            this.bind(querystring.parse(buffer));
            this.data = true;
            that.submit();
        });
        return;
    }
    throw new Error('Cannot handle request: ' + request.method);
}

/**
 * Bind submitted values to form definition.
 */
Form.prototype.bind(data) {
