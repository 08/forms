...
Form.load = function() {
    ...
    // Make use of node-formidable to parse the post request,
    // and populate the files array, if applicable.
...
Form.prototype.process = function(request, response) {
    ...
    var trackValidation = utils.whenDone(function() {
    ...
    trackValidation(function() {
        that.emit('validate', request, response, trackValidation);
    })();
};
...
Form.prototype.toHTML = function(iterator) {
    ...
    var html = fields.reduce(function(html, k) {
        if (that.fields[k].multipart === true) {
            multipart = true;
        }
...
util.inherits(InvalidFormToken, Error);
